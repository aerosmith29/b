...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/aerosmith29/b/main/ip > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/aerosmith29/b/main/ip | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/aerosmith29/b/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
